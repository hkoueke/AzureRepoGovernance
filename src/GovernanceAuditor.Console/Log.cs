...
internal static partial class Log
{
    ...
    [LoggerMessage(EventId = 1001, Level = LogLevel.Information, Message = "Analyse terminée : {Analyzed} dépôt(s) analysé(s), {Failed} en échec, {Skipped} écarté(s)")]
    public static partial void AnalysisCompleted(ILogger logger, int analyzed, int failed, int skipped);
    ...
    // Les dépôts vides ne sont pas connus à ce stade : ils ne se révèlent qu'à la
    // lecture des branches. Ce message ne compte donc que les dépôts désactivés.
    [LoggerMessage(EventId = 1007, Level = LogLevel.Information, Message = "{Analysable} dépôt(s) à interroger, {Disabled} désactivé(s)")]
    public static partial void RepositoriesRetained(ILogger logger, int analysable, int disabled);
